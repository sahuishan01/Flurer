...
//!
//! Nothing here runs during app startup — a watch is only ever registered
//! in response to a frontend request, so this module cannot affect the
//! launch spinner (see HANDOFF.md for why that distinction matters).
...
mod tests {
    ...
    // The watcher map is process-global, so every test that touches it
    // shares one fixture and runs in a single #[test] — separate tests
