...
/// the first chunk lands almost immediately, large enough that a
/// 100k-entry folder doesn't turn into thousands of IPC round trips.
...
/// read order would let rows land in the wrong place and then jump when
/// the sort finally ran — worse than waiting. What this actually buys is
/// twofold: the read runs on its own thread instead of blocking the main
/// thread the way a synchronous command does, and the result crosses the
/// IPC boundary in slices rather than as one enormous JSON string that has
/// to be serialized, copied and parsed before anything at all can be
/// painted.
#[tauri::command]
pub fn list_directory_streamed(
    ...
    std::thread::spawn(move || {
        ...
        // Drained from the front in place so entries are moved into each
